  ...
  "games": [
    {
      ...
      "home": "안산 그리너스",
      "away": "대구FC",
      "kickoff": "8/28(금) 19:30",
      "odds": { "승": 5.7, "무": 4.2, "패": 1.36 }
    }
  ]
}
```
...
| `updated_label` | 권장 | **배당 기준 시각** 짧은 표시용 (`8/28 19:00`) |
...
- 경기 수 상한은 없습니다. `kickoff` 빠른 순 → 같은 시각이면 `no` 순으로 정렬합니다.
- **축구 일반 승무패만** 담습니다. 핸디캡·언더오버·SUM(홀짝)·전반전, 그리고 축구 외
  종목은 제외합니다. 같은 경기의 일반/핸디캡/언더오버는 규정상 교차 조합이 안 되므로
  섞으면 계산이 틀려집니다.
- 배당이 하나도 없는 경기(발매 전 배당 미정)는 제외합니다. 프런트도 같은 조건으로
  한 번 더 걸러 냅니다.
