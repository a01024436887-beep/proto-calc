...
주소는 하나뿐이고, **상단 탭**으로 두 계산기를 오갑니다. 결과는 접히는 패널로 정리돼 있습니다.

1. **승무패 14경기 조합 계산기** — 아는 경기는 승·무·패 중 하나, 모르는 경기는
   "모름"을 체크하면 세 가지가 전부 조합에 들어갑니다.
2. **축구 전경기 배당 계산기** — 프로토 승부식 축구 경기의 승·무·패 배당을 보고
...
| `games[].type` | 필수 | `"일반"` / `"핸디캡"` / `"언더오버"` |
| `games[].handicap` | 일반·핸디캡 | 핸디캡일 때 숫자(홈 기준, 예: `-1.0`), 일반이면 `null` |
| `games[].line` | 언더오버 | 기준 구간값 (`2.5`, `3.5`) |
| `games[].home` / `.away` | 필수 | 팀명 |
| `games[].odds` | 필수 | 일반·핸디캡은 `승`/`무`/`패`, **언더오버는 `언더`/`오버` 두 개만** |
| `games[].league` / `.kickoff` | 선택 | 없으면 생략 |

주의사항:

- **한 원소 = 베트맨 경기번호 하나.** 같은 실제 경기라도 일반·핸디캡·언더오버는
  경기번호가 다르므로 **각각 별도 원소**로 담고, `match_key`만 같은 값을 넣습니다.
  한 경기에 기준값이 다른 핸디캡이 여럿, 구간이 다른 언더오버가 여럿 붙기도 합니다.
- 세 유형은 규정상 서로 교차 조합이 안 됩니다. 프런트가 `match_key`로 묶어 하나를
  고르면 나머지를 자동으로 잠그므로, **`match_key`를 정확히 넣는 게 중요**합니다.
- **배당은 반올림하지 말고 원본 소수점 그대로** 넣습니다.
- 항목 수 상한은 없습니다. `kickoff` 빠른 순 → 같은 경기 안에서는 일반 → 핸디캡 →
  언더오버 → `no` 순으로 정렬합니다.
- **축구의 일반·핸디캡·언더오버만** 담습니다. SUM(홀짝)·전반전, 그리고 축구 외 종목은
  제외합니다.
- 핸디캡 기준값이 `.5` 단위거나 언더오버면 무승부가 없어 `odds`에서 `"무"`가 빠집니다.
- **언더오버 배당 방향**: 베트맨 응답의 `winAllot`이 **언더**, `loseAllot`이 **오버**입니다.
  응답이 `winTxt='언더'` / `loseTxt='오버'`로 직접 라벨을 주고, 기준선이 오를수록
  언더 배당이 내려가고 오버가 올라가는 것으로도 교차 확인했습니다. 손으로 고칠 때
  이 둘을 바꾸면 계산이 통째로 틀어지니 주의하세요.
